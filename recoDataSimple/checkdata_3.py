...
# FILTER 4: 2D Torsion mapping and dynamic Lindhard cut
print("\tMapping 2D Torsion (Local Theta_0 vs X and Y)...")

# Define the checkerboard grid (sub-samples)
nx_slices = 10 # Adjust based on your height/statistics
ny_slices = 10 # Adjust based on your width/statistics

# Create 3D histograms to avoid looping over RDataFrame
# X-axis: theta_in, Y-axis: d0_x, Z-axis: d0_y
h3_all = df_phys.Histo3D(("h3_all", "", 1000, -100, 100, nx_slices, x_min, x_max, ny_slices, y_min, y_max), "thetaIn_x", "Tracks.d0_x", "Tracks.d0_y").GetValue()
h3_chan = df_phys.Filter(f"Deltatheta_x > {preliminary_cut}").Histo3D(("h3_chan", "", 1000, -100, 100, nx_slices, x_min, x_max, ny_slices, y_min, y_max), "thetaIn_x", "Tracks.d0_x", "Tracks.d0_y").GetValue()

# 2D Histogram to store the final map
h2_torsion_map = ROOT.TH2D("h2_torsion_map", "2D Torsion Map; x at crystal surface [mm]; y at crystal surface [mm]; angle shift #theta_{0} [#murad]", nx_slices, x_min, x_max, ny_slices, y_min, y_max)
...
for ix in range(1, nx_slices + 1):
    for iy in range(1, ny_slices + 1):
        ...
        while current_theta <= scan_max:
            bin_min = h1_all.FindBin(current_theta - theta_L / 2.0)
            bin_max = h1_all.FindBin(current_theta + theta_L / 2.0)
            
            n_tot_test = h1_all.Integral(bin_min, bin_max)
            n_ch_test  = h1_chan.Integral(bin_min, bin_max)
            
            eff_test = 0.0
            if n_tot_test > 15: # Minimum statistics safeguard for 2D
...
torsion_fit_2d = ROOT.TF2("torsion_fit_2d", "[0] + [1]*x + [2]*y", x_min, x_max, y_min, y_max)
...
torsion_plot_2d = ROOT.TF2("torsion_plot_2d", f"{theta_0_baseline} + {tau_x}*y + {tau_y}*x", y_min, y_max, x_min, x_max)
torsion_plot_2d.SetTitle("Continuous 2D Torsion Map; y at crystal surface [mm]; x at crystal surface [mm]; angle shift #theta_{0} [#murad]")
...
h2_torsion_map.GetZaxis().SetRangeUser(-80, 80) # Adjust based on expected torsion range
h2_torsion_map.Draw("COLZ")
c_torsion_2d.Update()
# c_torsion_2d.SaveAs(f"plots_{file}/torsion_map_2d.png")

# Apply the Dynamic 2D Lindhard Cut
# The acceptance window now dynamically adjusts based on BOTH x and y impact positions
cut = f"abs(thetaIn_x - ({theta_0_baseline} + {tau_x} * Tracks.d0_x + {tau_y} * Tracks.d0_y)) <= {theta_L / 2.0}"
df_phys = df_phys.Filter(cut, "2D Torsion-corrected Lindhard cut")
P4 = df_phys.Count().GetValue()

best_theta_0 = theta_0_baseline 

print(f"Filter 4 (2D Dynamic Lindhard cut): {((P3 - P4)/P3*100):.2f}% events out of {P3} got discarded.")
print("="*50)
