...
def angular_scan_plot(p_in, p_out, label, angle_unit='urad'):
    ...
    # angular scan
    axs[0].scatter(theta_in * scale, dtheta * scale, s=6, alpha=0.6)
    axs[0].axhline(0, color='gray', lw=0.8, ls='--')
    axs[0].set_xlabel(rf'$\theta_{{in}}$ [${unit_str}$]')
    axs[0].set_ylabel(rf'$\Delta\theta = \theta_{{out}} - \theta_{{in}}$ [${unit_str}$]')
    axs[0].set_title(f'Angular scan – {label}')
    axs[0].grid(alpha=0.3)

    # histo 1d delta theta
    axs[1].hist(dtheta * scale, bins=100, color='#1f77b4', edgecolor='white', linewidth=0.3)
    axs[1].set_xlabel(rf'$\Delta\theta$ [${unit_str}$]')
    axs[1].set_ylabel('Counts')
    axs[1].set_title(f'$\Delta\\theta$ distribution – {label}')
